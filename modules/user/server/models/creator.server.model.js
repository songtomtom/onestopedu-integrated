 ...
 */
CreatorSchema.methods.hashPassword = function(password) {
 ...
 */
CreatorSchema.methods.authenticate = function(password) {
 ...
 */
CreatorSchema.statics.findUniqueUsername = function(username, suffix, callback) {
 ...
 */
CreatorSchema.statics.generateRandomPassphrase = function() {
 ...
 */
CreatorSchema.statics.seed = function(doc, options) {
  ...
  return new Promise((resolve, reject) => {
    ...
    function add(skip) {
      return new Promise((resolve, reject) => {
        ...
        if (skip) {
          return resolve({
            message: chalk.yellow(`Database Seeding: Creator ${doc.username} skipped`)
          });
        }

        const creator = new Creator(doc);
        ...
        creator.save((err) => {
          ...
          return resolve({
            message: 'Database Seeding: Creator added'
          });
        });
      });
    }
 ...
 */
CreatorSchema.plugin(autoIncrement.plugin, {
  model: 'Creator',
  field: 'counters',
  startAt: 1,
  incrementBy: 1,
 ...
 * Schema Compile
 */
mongoose.model('Creator', CreatorSchema, 'users');
