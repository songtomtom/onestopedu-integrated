 ...
 * Schema
 */
const TutorManagerSchema = new mongoose.Schema({
  username: {
    type: String,
    required: 'Please fill in a username',
    lowercase: true,
    trim: true,
    validate: [validateUsername, 'Please fill in your username']
  },
  email: {
    type: String,
    lowercase: true,
 ...
 */
TutorManagerSchema.pre('save', function(next) {
 ...
 */
TutorManagerSchema.pre('validate', function(next) {
 ...
 */
TutorManagerSchema.methods.hashPassword = function(password) {
 ...
 */
TutorManagerSchema.methods.authenticate = function(password) {
 ...
 */
TutorManagerSchema.statics.findUniqueUsername = function(username, suffix, callback) {
 ...
 */
TutorManagerSchema.statics.generateRandomPassphrase = function() {
 ...
 */
TutorManagerSchema.statics.seed = function(doc, options) {
  ...
  return new Promise((resolve, reject) => {
    ...
    function add(skip) {
      return new Promise((resolve, reject) => {
        ...
        if (skip) {
          return resolve({
            message: chalk.yellow(`Database Seeding: TutorManager ${doc.username} skipped`)
          });
        }

        const tutorManager = new TutorManager(doc);

        // user.password = passphrase;
        const password = 'qwe';
        tutorManager.password = password;
        ...
        tutorManager.save((err) => {
          ...
          return resolve({
            message: 'Database Seeding: TutorManager added'
          });
        });
      });
    }
 ...
 */
TutorManagerSchema.plugin(autoIncrement.plugin, {
  model: 'TutorManager',
  field: 'counters',
  startAt: 1,
  incrementBy: 1,
 ...
 * Schema Compile
 */
mongoose.model('TutorManager', TutorManagerSchema, 'users');
