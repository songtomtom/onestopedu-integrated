(function() {

  /**
   * Module Configuration
   */
  angular
    .module('schedule')
    .controller('ApplyGuestLeveltestController', ApplyGuestLeveltestController);

  /**
   * Dependency Injection
   */
  ApplyGuestLeveltestController.$inject = [
    '$scope',
    '$window',
    '$state',
    'leveltestResolve',
    'guestResolve',
    'Authentication',
    'TalkDream',
    'dayPickerConfig',
    'toastr'
  ];

  /**
   * Configuring the apply leveltest controller
   */
  function ApplyGuestLeveltestController(
    $scope,
    $window,
    $state,
    leveltest,
    guest,
    Authentication,
    TalkDream,
    dayPickerConfig,
    toastr
  ) {

    const vm = this;

    vm.leveltest = leveltest;
    vm.application = {
      minutes: 10,
      productType: 'telephone',
      skill: 'novice'
    };

    vm.guest = guest;

    vm.apply = apply;
    vm.auth = Authentication;
    vm.datetimePickerOptions = angular.copy(dayPickerConfig);

    /**
     * Regist leveltests
     */
    function apply(isValid) {

      if (!isValid) {
        $scope.$broadcast('show-errors-check-validity', 'vm.form.leveltestForm');
        return false;
      }

      /**
       * Regist leveltests
       */
      if ($window.confirm('Are you sure you want to apply new leveltest?')) {
        vm.guest.providers = vm.leveltest.providers;
        vm.guest.$signup()
          .then(onSignupGuestSuccess)
          .catch(onSignupGuestError);

      }

      /**
       * Sign up guest success callback
       */
      function onSignupGuestSuccess(guest) {
        vm.leveltest.user = guest._id;
        vm.leveltest.application = vm.application;
        vm.leveltest.$save()
          .then(onSaveLeveltestSuccess)
          .catch(onSaveLeveltestError);
      }

      /**
       * Sign up guest error callback
       */
      function onSignupGuestError(err) {
        toastr.error(err.data.message, 'Error', {
          timeOut: 0
        });
      }

      /** Save Leveltest success callback */
      function onSaveLeveltestSuccess(leveltest) {
        TalkDream.sendMessageToApplyLeveltest(
            leveltest.application.productType,
            leveltest.user,
            leveltest._id
          )
          .then(onSendMessageSuccess)
          .catch(onSendMessageError);
      }

      /** Save Leveltest error callback */
      function onSaveLeveltestError(err) {
        toastr.error(err.data.message, 'Error', {
          timeOut: 0
        });
      }

      /** Send to apply to leveltest success callback */
      function onSendMessageSuccess() {
        toastr.success(moment().format('YYYY/MM/DD HH:mm:ss'), 'Success');
        $state.go('guest.view.leveltest.list.detail', {
          guestId: leveltest.user,
          leveltestId: leveltest._id
        });
      }

      /** Send to apply to leveltest success callback */
      function onSendMessageError(err) {
        toastr.error(err.data.message, 'Error', {
          timeOut: 0
        });
      }
    }
  }

}());
