(function() {
  ...
  angular
    .module('schedule')
    .controller('DetailMemberLeveltestController', DetailMemberLeveltestController);

  /**
   * Dependency Injection
   */
  DetailMemberLeveltestController.$inject = [
    '$scope',
    '$window',
    '$state',
    '$timeout',
   ...
   */
  function DetailMemberLeveltestController(
    $scope,
    $window,
    $state,
    $timeout,
    ...
    function downloadFile() {
      ...
      FileItem.downloadFileItem(vm.assessment.recordFile._id)
        .then(onDownloadFileItemSuccess)
        .catch(onDownloadFileItemError);

      // File items upload log create success callback
      function onDownloadFileItemSuccess(fileItem) {}

      // File items upload log create error callback
      function onDownloadFileItemError(err) {
        toastr.error(err.data.message, 'Error', {
          timeOut: 0
        });
      }
     ...
     */
    function sendMail() {
      if ($window.confirm('Would you like to send an email this result to member?')) {
