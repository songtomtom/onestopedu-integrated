  ...
  function Menu() {
    ...
    function addMenuItem(menuId, options = {}) {
      ...
      service.menus[menuId].items.push({
        ...
        contentsUrl: options.contentsUrl || '',
    ...
    function shouldRender(user) {
      ...
      const matchingRoles = user.roles.filter((userRole) => {
    ...
    /**
     * Adding the topbar and sidebar menu
     */
    function init() {
      addMenu('sidebar', {
        roles: ['*']
    ...
    function removeMenuItem(menuId, menuItemState) {
      ...
      service.menus[menuId].items = service.menus[menuId].items.filter((item) => {
