...
function validateSessionSecret(config, testing) {
  ...
  if (config.sessionSecret === 'INTEGRATED-SESSION-SECRET') {
    if (!testing) {
...
function initGlobalConfigFolders(config, assets) {
  ...
  config.directories.client = getGlobbedPaths(path.join(process.cwd(), 'modules/*/client/'), process.cwd().replace(new RegExp(/\\/g), '/'));
}
...
module.exports = (function() {
  ...
  /** Read package.json for INTEGRATED project information */
  config.version = require(path.resolve('./package.json')).version;
  /**
